...
class MCTS:
    ...
    def build_tree(self, node):
        ...
        while node.untried_actions == [] and node.children:
            # From the current state s, choose the action with highest UCB1
            action = self.selection(node)
            child = next(c for c in node.children if c.action == action)
            node = child
            current_depth += 1

        # 2. Expansion: if there are untried actions, expand one
        if node.untried_actions:
            action = node.untried_actions.pop()
            node = self.expand(action, node)

        # 3. Simulation / Rollout
        # Run a simulated playout from current until a result is achieved.
        rollout_return = self.rollout(node.state, current_depth)
